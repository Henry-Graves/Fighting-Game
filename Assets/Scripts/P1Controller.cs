...
public class P1Controller : MonoBehaviour
{
	...
	public GameObject enemyRoundWin;
	public GameObject enemyMatchWin;
	...
	void Start()
	{
		...
		controllerCharacter = GameObject.Find("Player1");
	...
	{
		if (StatsManager.P1Health <= 0 && !alreadyDead)
		{
			StartCoroutine(KO());
			alreadyDead = true;
		}
	...
	{
		if (gameObject.transform.position.y < 0.181f && !animState.IsTag("Jumping"))
			isGrounded = true;
		...
		if (Input.GetAxis("Vertical") > 0 && !animState.IsTag("Jumping") && !animState.IsTag("KnockedOut") && isGrounded)
		{
		...
		if (animState.IsTag("Blocking") && Input.GetAxis("Block") == 0)
		{
			anim.SetBool("Block", false);
		}
	}
	...
	{
		if (!animState.IsTag("Jumping") && isGrounded)
		{
			anim.SetTrigger("Jump");
			isGrounded = false;
			yield return new WaitForSeconds(0.2f);
			Debug.Log("isGrounded= " + isGrounded);
			rigidBody.AddForce(new Vector3(0.0f, 2.0f, 0.0f) * 0.5f, ForceMode.Impulse);
		}
		else
			yield break;
	}
	...
	IEnumerator KO()
	{
		...
		if (facingLeft)
		{
			StartCoroutine(P2Win(anim.GetCurrentAnimatorStateInfo(1).length));
		}
		else
		{
			StartCoroutine(P2Win(anim.GetCurrentAnimatorStateInfo(0).length));
		}

		yield return new WaitForSeconds(0.04f);
		// gameObject.GetComponent<P1Controller>().enabled = false;
	}

	// Displays win screen after death animation
	public IEnumerator P2Win(float delay)
	{
		StatsManager.timerWinEnabled = false;
		yield return new WaitForSeconds(delay);
		StatsManager.P1Hearts -= 1;

		// check if out of hearts, if so then display gameoverscreen
		if (StatsManager.P1Hearts == 0)
		{
			enemyMatchWin.SetActive(true);
			PauseMenu.P2Match = true;
		}
		else
		{
			enemyRoundWin.SetActive(true);
			PauseMenu.P2Round = true;
